...
SRC_REPO="git@github.com:CDXi-solutions/ocean-colour.git"
SRC_BRANCH="binaries"
DEST_REPO_LOCAL="/home/kostageo/Projects/HELOISA-software/integration-tests/water-monitoring-integration-tests"
DEST_SUBPATH="water-quality/ocean-colour/run-files"
COMMIT_MSG="Update ocean colour run-files (workflow_ocean_colour.cwl, inputs.yml)"
VERBOSE=0
FORCE=0   # if 1, stash local changes in dest repo to allow pull
QUIET_GIT=--quiet

# Replacement defaults
REWRITE_FROM='dockerPull: ghcr.io/cdxi-solutions/oceanimg:bin'
REWRITE_TO='dockerPull: "ghcr.io/hellenicspacecenter/water-monitoring-integration-tests/oceancolour-service:latest"'
...
# Stage changes
git -C "$DEST_REPO_LOCAL" add "$DEST_DIR/oceancolour.cwl" "$DEST_DIR/inputs.yml"
