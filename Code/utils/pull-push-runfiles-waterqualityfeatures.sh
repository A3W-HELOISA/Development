...
SRC_REPO="git@github.com:CDXi-solutions/hydroscan.git"
SRC_BRANCH="binaries"
DEST_REPO_LOCAL="/home/kostageo/Projects/HELOISA-software/integration-tests/water-monitoring-integration-tests"
DEST_SUBPATH="water-quality/water-quality-features/run-files"
COMMIT_MSG="Update water quality features run-files (workflow_hydroscan.cwl.cwl, inputs.yaml)"
VERBOSE=0
...
SRC_MUDDY="$(find_one 'workflow_hydroscan.cwl')"
SRC_INPUTS="$(find_one 'inputs.yml')"

# Rewrite dockerPull line(s) in reference_muddy.cwl
log "Rewriting dockerPull line(s) in reference_muddyworkflow_hydroscan.cwl…"
before_count=$(grep -Fxc "$REWRITE_FROM" "$SRC_MUDDY" || true)
MODIFIED_CWL="$WORKDIR/workflow_hydroscan.modified.cwl"
...
install -m 0644 "$MODIFIED_CWL" "$DEST_DIR/hydroscan.cwl"
install -m 0644 "$SRC_INPUTS" "$DEST_DIR/inputs.yml"

# Stage changes
git -C "$DEST_REPO_LOCAL" add "$DEST_DIR/hydroscan.cwl" "$DEST_DIR/inputs.yml"
