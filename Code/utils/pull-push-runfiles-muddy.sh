...
SRC_MUDDY="$(find_one 'reference_muddy.cwl')"
SRC_INPUTS="$(find_one 'inputs.yaml')"

# Rewrite dockerPull line(s) in reference_muddy.cwl
log "Rewriting dockerPull line(s) in reference_muddy.cwl…"
before_count=$(grep -Fxc "$REWRITE_FROM" "$SRC_MUDDY" || true)
MODIFIED_CWL="$WORKDIR/reference_muddy.modified.cwl"
...
install -m 0644 "$MODIFIED_CWL" "$DEST_DIR/muddy.cwl"
install -m 0644 "$SRC_INPUTS" "$DEST_DIR/inputs.yaml"

# Stage changes
git -C "$DEST_REPO_LOCAL" add "$DEST_DIR/muddy.cwl" "$DEST_DIR/inputs.yaml"
