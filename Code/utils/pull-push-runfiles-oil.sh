...
DEST_SUBPATH="water-quality/extreme-events/run-files"
COMMIT_MSG="Update extreme-events run-files (muddy.cwl, inputs.yaml)"
VERBOSE=0
FORCE=0   # if 1, stash local changes in dest repo to allow pull
QUIET_GIT=--quiet

# Replacement defaults
REWRITE_FROM='dockerPull: "ghcr.io/cdxi-solutions/oilspill-service:bin"'
REWRITE_TO='dockerPull: "ghcr.io/hellenicspacecenter/water-monitoring-integration-tests/oilspill-service:latest"'
...
SRC_MUDDY="$(find_one 'reference_oil.cwl')"
SRC_INPUTS="$(find_one 'inputs.yaml')"

# Rewrite dockerPull line(s) in reference_oil.cwl
log "Rewriting dockerPull line(s) in reference_oil.cwl…"
before_count=$(grep -Fxc "$REWRITE_FROM" "$SRC_MUDDY" || true)
MODIFIED_CWL="$WORKDIR/reference_oil.modified.cwl"
...
install -m 0644 "$MODIFIED_CWL" "$DEST_DIR/oil.cwl"
install -m 0644 "$SRC_INPUTS" "$DEST_DIR/inputs.yaml"

# Stage changes
git -C "$DEST_REPO_LOCAL" add "$DEST_DIR/oil.cwl" "$DEST_DIR/inputs.yaml"
